...
def pretrain(device, model, criterion, args, train_loader, test_loader, model_root_dir, result_path, save_model=False, save_result=True):
    ...
    not_improved_count = 0
    # best_metric_record = {}, {}
    best_train_record, best_test_record = {}, {}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_loader is not None:
            ...
            scheduler.step(test_record["test_f1"])
            ...
            # early stopping
            epoch_metric = test_record["test_f1"]
            if epoch_metric > best_metric:
                best_metric = epoch_metric
                best_epoch = epoch
                # best_metric_record = train_record, test_record
                best_train_record, best_test_record = train_record, test_record
                not_improved_count = 0

                if save_model:
                    torch.save(model.state_dict(), os.path.join(model_root_dir, f"model_best.pth"))
            else:
                not_improved_count += 1
